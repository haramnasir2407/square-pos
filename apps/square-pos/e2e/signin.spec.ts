...
test("Sign in with Square", async ({ page }) => {
  ...
  await page
    .locator('[id="2fa-post-login-promo-sms-remind-me-btn"]')
    .getByRole("button", { name: "Remind me next time" })
    .click();
  await page
    .locator('[id="2fa-post-login-promo-opt-out-modal-continue"]')
    .getByRole("button", { name: "Continue to Square" })
    .click();
  await expect(
    page.getByRole("heading", { name: "Sandbox test accounts" })
  ).toBeVisible();
  ...
  await page1
    .getByRole("button", { name: "Success checkmark Sign in" })
    .click();
  await page1.getByRole("button", { name: "Allow" }).click();
  console.log(await page.content());
  await expect(
    page1.getByRole("heading", { name: "Welcome back, Default Test" })
  ).toBeVisible();
});
